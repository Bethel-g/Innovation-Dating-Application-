...
export default function AdminModeration() {
  ...
  const loadData = async () => {
    ...
    try {
      if (tab === 'reports') {
        const res = await api.get('/admin/reports', { params: { page: 1, limit: 20, status: 'pending' } });
        setReports(res.data.reports);
      } else {
        const res = await api.get('/admin/flagged-messages', { params: { page: 1, limit: 20 } });
  ...
  return (
    <div>
      <div className="page-header"><h1>Content Moderation</h1></div>
      <div className="tabs">
        <button className={`tab ${tab === 'reports' ? 'active' : ''}`} onClick={() => setTab('reports')}>Reports</button>
        <button className={`tab ${tab === 'messages' ? 'active' : ''}`} onClick={() => setTab('messages')}>Flagged Messages</button>
      </div>
      {loading ? <div className="empty-state"><div className="spinner" /></div> : tab === 'reports' ? (
        <div className="card">{
