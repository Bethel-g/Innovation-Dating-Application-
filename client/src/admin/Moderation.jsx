...
export default function Moderation() {
  ...
  const loadData = async () => {
    ...
    try {
      if (tab === 'reports') {
        const res = await adminAPI.get('/admin/reports', { params: { page, limit: 20, status: 'pending' } });
        setReports(res.data.reports);
      } else {
        const res = await adminAPI.get('/admin/flagged-messages', { params: { page, limit: 20 } });
  ...
  const resolveReport = async (id, status, actionTaken) => {
    try {
      await adminAPI.put(`/admin/reports/${id}`, { status, actionTaken, resolution: `Action: ${actionTaken}` });
  ...
  const moderateMessage = async (id, action) => {
    try {
      await adminAPI.put(`/admin/messages/${id}/moderate`, { action });
