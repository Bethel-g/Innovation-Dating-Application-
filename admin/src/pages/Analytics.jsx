...
export default function Analytics() {
  ...
  const totalProjects = data?.projectActivity?.reduce((sum, d) => sum + d.count, 0) || 0;
    ...
    <div className="analytics-page page-transition">
      ...
      <div className="grid grid-4">
        <div className="card stat-card">
          ...
          <div className="stat-label">New Professionals</div>
        </div>
        <div className="card stat-card">
        ...
        <div className="card stat-card">
          ...
          <div className="stat-label">Project Activities</div>
        </div>
      </div>

      <div className="grid grid-2" style={{ marginTop: 24 }}>
        <div className="card">
          <h3>Professional Growth (by day)</h3>
              ...
              <div className="bar-chart">
                {data.userGrowth.slice(-14).map((d, i) => (
                  <div key={i} className="bar-wrapper">
                    ...
                    <span className="bar-label">{d.id?.slice(-5)}</span>
                  </div>
                ))}
              </div>
        ...
        <div className="card">
          ...
          <div className="gender-dist">
            {data?.genderDistribution?.map(g => (
              <div key={g.id} className="gender-item">
                <span className="gender-label">{g.id}: {g.count}</span>
      ...
      <style>{`
        ...
        .bar { width: 100%; background: linear-gradient(to top, #EF4444, #F97316); border-radius: 4px 4px 0 0; transition: height 0.3s; min-height: 4px; }
